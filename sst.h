...
namespace sst {
  ...
  public:
    ...
    SST(const vector<int> &_members, int _node_rank, failure_upcall_t failure_upcall=NULL) :
      SST(_members, _node_rank, std::pair<std::tuple<>, std::vector<row_predicate_updater_t> > { }, failure_upcall) {
    }

    template<typename ExtensionList, typename ... RestFunctions>
    SST(const vector<int> &_members, int _node_rank, const PredicateBuilder<Row, ExtensionList> &pb, RestFunctions ... named_funs, failure_upcall_t failure_upcall=NULL) :
      SST(_members, _node_rank, constructor_helper<0>(pb, named_funs...), failure_upcall) {
    }
     ...
     * this code is running.
     */
    SST(const vector<int> &_members, int _node_rank, std::pair<decltype(named_functions), std::vector<row_predicate_updater_t> >, failure_upcall_t _failure_upcall=NULL);
