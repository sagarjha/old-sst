...
/** Initializes the TCP subsystem of %SST, which is necessary for out-of-band
 * communications. */
void tcp_initialize(uint32_t node_rank,
                    const std::map<uint32_t, std::string> &ip_addrs);
/** Exchanges a byte of data with the given node over TCP; blocks until the
 * remote node responds with its own sync(). */
