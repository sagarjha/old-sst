...
namespace sst {
  ...
  enum class PredicateType { ONE_TIME, RECURRENT, TRANSITION };
  ...
  public:
    ...
    void insert(pred_read f, trig_read g, PredicateType type=PredicateType::ONE_TIME);
  };
  ...
  template <class Row>
  void Predicates_read<Row>::insert(pred_read f, trig_read g, PredicateType type) {
    list <trig_read> g_list;
    g_list.push_back (g);
    if (type == PredicateType::ONE_TIME) {
      one_time_predicates.push_back (pair <pred_read, list<trig_read> > (f, g_list));
    }
    else if (type == PredicateType::RECURRENT) {
      recurrent_predicates.push_back (pair <pred_read, list<trig_read> > (f, g_list));      
    }
    else {
      transition_predicates.push_back (pair <pred_read, list<trig_read> > (f, g_list));
    }
  }

  // insert into different lists based on the type
  template <class Row>
  void Predicates_write<Row>::insert(pred_write f, trig_write g, PredicateType type) {
    list <trig_write> g_list;
    g_list.push_back (g);
    if (type == PredicateType::ONE_TIME) {
      one_time_predicates.push_back (pair <pred_write, list<trig_write> > (f, g_list));
    }
    else if (type == PredicateType::RECURRENT) {
      recurrent_predicates.push_back (pair <pred_write, list<trig_write> > (f, g_list));      
    }
    else {
      transition_predicates.push_back (pair <pred_write, list<trig_write> > (f, g_list));
    }
  }
}
