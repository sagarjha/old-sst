 ...
 */
	 ...
	 *
	 * The Row is just the user's Row (as in SST).  "Metadata" is a struct which contains handy type definitions (see its definition below)
	 * TypeList is a list of types; it works similarly to NamedRowPredicates, but with fewer handy accessors.
	 *
	...
	struct PredicateBuilder<Row,TypeList<Metadata>> {
		...
		/**
		 * This is the current predicate the user provided (or that we've built up to now).
		 * Please note that the parameter supplied to this function's two arguments ("Row" and "Row_Extension") will in practice *both* be the SST's internal row.
		...
		/**
		 * If the user has named this Predicate, then this will be a tuple consisting of a
		 * single function which calculates the predicate's result.  It's basically the
		 * user-facing interface to the predicate. Gets the result of the function of this name.
		 ...
		 *
		 * In practice, there may be several getters if the user names the predicate at several layers.
	...
	struct PredicateBuilder {
		...
		/**
		 * "Uniqueness Tag" is just the name of this predicate.  If this predicate doesn't have a name yet,
		 * it's just the name of the dependency-predicate; for example, if this is E(pred), then it would be pred's name.
		...
		using Getters = std::decay_t<
			decltype(std::tuple_cat(
						 ...
						 std::conditional_t</*if*/ hd::has_name::value, /*then*/ std::tuple<std::function<row_entry (InternalRow)> >, /*else*/ std::tuple<> >
						 >(),
		...
		using num_getters = typename std::tuple_size<Getters<int/*dummy param*/> >::type;
		...
		/**
		 * see the discussion of this function in the base case of NamedPredicate, many lines up.
		 * @return
		 */
		template<typename T>
	...
	namespace predicate_builder {
		...
		//but we're the tail.
		template<typename NameEnum, NameEnum Name, typename Row, typename fun_return, typename Up, typename Get>
...
#define rowpred_template_arg(x...) rowpred_template_arg_t<decltype(x)>
		...
		auto Min(const PredicateBuilder<Row, TypeList<hd,tl...> >& pb) {
			...
			auto updater_f = [curr_pred_raw](volatile /*Row_Extension that is mine*/ auto& my_row,
