 ...
 */
int get_socket(uint32_t rank) { return sockets[rank]; }
 ...
 *
 * @param sock The file descriptor of the socket to use for the remote
 *connection
...
/**
 * Establishes TCP connections with all the nodes whose IP addresses are
 *specified.
 *
 * @param ip_addrs A map of node ranks to the IP addresses of the nodes in the
 *system,
 * indexed by the key node rank. Its size gives the number of nodes in the
 *system.
...
void establish_tcp_connections(const map<uint32_t, string> &ip_addrs,
    ...
    for(auto it = ip_addrs.rbegin(); it != ip_addrs.rend(); ++it) {
        if(it->first > node_rank) {
            ...
            cout << "connected to node rank " << it->first << endl
                 << endl;
            ...
            cout << "connected to node rank " << p.second << endl
                 << endl;
